...
class VisionService {
  ...
  private init() {
     // Configuration: Usage of .cpuAndGPU is CRITICAL to prevent ANE (Neural Engine) crashes.
  ...
  func classify(image: UIImage) async throws -> VisionAnalysisResult {
    ...
    return try await Task.detached(priority: .userInitiated) { [weak self] in
      guard let self = self else { throw NSError(domain: "VisionService", code: -1, userInfo: [NSLocalizedDescriptionKey: "Service deallocated"]) }
      ...
      try await withThrowingTaskGroup(of: VisionTaskResult.self) { group in
        ...
        // 1. Task: YOLOv3 (Object Detection)
        if let yolo = yolo {
          group.addTask {
        ...
        // 2. Task: MobileNetV2 (Classification)
        if let mob = mob {
          group.addTask {
            var resultsVec: [ScoredTag] = []
        ...
        // 3. Task: OCR (Text Recognition) - Executed for system stability check but unused for naming per requirements
        group.addTask {
          var texts: [String] = []
      ...
      func isConsistent(ocr: String, tags: [ScoredTag]) -> Bool {
        ...
        // 1. Check for specific ignored phrases
        if ocrLower.contains("not found") || ocrLower.contains("no item") { return false }
        
        // 2. Check for semantic match with top ML tags (loose check)
